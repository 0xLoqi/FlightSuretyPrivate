...
contract FlightSuretyApp {
    ...
    function getInsuredPassengers(
        ...
        uint256 passengerCount = flightSuretyData.getInsuredPassengerCount(
            flightKey
        );
        address[] memory insuredPassengers = new address[](passengerCount);
        for (uint256 i = 0; i < passengerCount; i++) {
            insuredPassengers[i] = flightSuretyData.getInsuredPassenger(
                flightKey,
                i
            );
        }
    ...
    function processFlightStatus(
        ...
        if (statusCode == STATUS_CODE_LATE_AIRLINE) {
            address[] memory insuredPassengers = getInsuredPassengers(
                flightKey
            );
